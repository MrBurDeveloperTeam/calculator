...
export default function CatMascot({ onCatClick, disabled = false }) {
  ...
  // The already-resolved Phase-2 candidate (from Dashboard.tsx via the
  // read-only bridge context) while it's the active dialogue — mirrored
  // into React state (below) for render-time reads, following the same
  // ref+state split already used for isDialogActive.
  ...
  // time) — the dialogue persists once shown regardless of later
  // savedPlans changes, so the action it runs must stay the one that was
  // valid when it was shown (and correctly bound to the SAME saved plan —
  // see App.tsx's closure-safety comment), even if personalizedInsightState
  // has since gone back to not_ready.
  ...
  // reads. Two-way, both always a real object (never null — the Profit
  // publisher is app-wide and always co-mounted with this component, see
  // PersonalizedInsightBridge.tsx's header): {status:'not_ready'}
  // (calculatorDataStatus/ownership not yet satisfied, or App.tsx's own
  // publish effect hasn't run yet on this exact render) |
  // {status:'ready', candidate, onAction} (resolution concluded).
  const bridgeEntryRef = useRef({ status: 'not_ready' });
  ...
  // Read-only: the already-resolved Phase-2 candidate published app-wide by
  // App.tsx, regardless of currentView — see
  // PersonalizedInsightBridge.tsx's header.
  ...
  // Reads the bridge's explicit two-way state (never treats a bare
  // `not_ready` candidate as proof of readiness) and only ever decides once
  // per activation, regardless of query speed:
  //
  //   bridge.status === 'not_ready' -> the app-wide Profit source hasn't
  //                                  satisfied its readiness+ownership gate
  //                                  yet (still loading, or resolved for a
  //                                  different user), OR App.tsx's own
  //                                  publish effect simply hasn't run yet
  //                                  on this very first render — leave the
  //                                  decision unresolved either way; this
  //                                  function will be called again by the
  //                                  reactive effect below once the bridge
  //                                  changes. Never falls through to
  //                                  Welcome Back from this branch.
  //   bridge.status === 'ready'  -> savedPlans have genuinely resolved
  ...
  //
  // There is no longer a `bridge === null` ("publisher absent") branch —
  // the Profit publisher is app-wide and always co-mounted with this
  // component for the entire authenticated app lifetime, so that state no
  // longer exists (see PersonalizedInsightBridge.tsx's header).
  //
  ...
  const arbitrateReturningUser = async (uid) => {
    ...
    // status === 'ready'
    //
    // Eligibility is now two SEPARATE checks, deliberately not merged
    // behind one persisted-storage function: shownCandidateKeysRef
    // (in-memory, this mount only) prevents repeatedly reopening the same
    // candidate within one activation; isDialogueDismissed (localStorage,
    // cross-tab, survives reload) is the ONLY thing that suppresses a
    // candidate across reloads. A candidate merely shown and never
    // explicitly Closed/CTA'd is NOT in localStorage, so it remains
    // eligible on the next fresh mount (reload).
    const candidate = bridgeState.candidate;
    if (candidate?.dedupeKey && !shownCandidateKeysRef.current.has(candidate.dedupeKey) && !isDialogueDismissed(uid, candidate.dedupeKey)) {
  ...
  // changes (Dashboard mounting/unmounting, or its calculatorDataStatus/
  // ownership settling). No-ops immediately via the
  ...
  // dedupeKey) is dismissed (Close or CTA) in another same-origin Profit
  // Calculator tab, that tab's write to localStorage fires the native
  ...
  useEffect(() => {
    const initDialog = async () => {
      ...
      // whichever of Dashboard's saved-plan data or this Welcome Back
      // fetch happens to resolve first. If the bridge is still 'not_ready'
      // at this exact moment, arbitrateReturningUser deliberately does
    ...
    <>
      ...
      >
        ...
        {/* Phase-2 proactive personalized reminder — reuses the exact same
            ...
            reuses Dashboard's existing action handler verbatim via the
            bridge. */}
        <AnimatePresence mode="wait">
                  ...
                  {activeCandidate.action && (
                    <button
                      onClick={(e) => {
                        ...
                        // Dashboard's existing action — never a new action
                        // derived here. Uses the action captured at
                        // adoption time (not a fresh read of the bridge),
                        // since the dialogue persists even if the bridge
                        // has since gone back to null/not_ready, and
                        // guarantees the CTA opens the SAME plan the Cat
                        // displayed (see Dashboard.tsx's closure-safety
                        // comment).
        ...
        <AnimatePresence mode="wait">
          {!disabled && !isDialogActive && meowMsg && (
            <motion.div
              ...
              className="max-w-[calc(100vw-2rem)] px-4 py-2.5 bg-white border border-slate-200 rounded-lg shadow-sm relative pointer-events-auto mb-4 cursor-default md:max-w-none md:mr-1"
            >
