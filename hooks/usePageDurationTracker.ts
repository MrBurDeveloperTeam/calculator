...
/**
 * Tracks how long the user spends on each view of the Dental Profit
 * Calculator app (Dashboard, Settings, History, each cost-category
 * calculator, Procedure Builder) and reports the duration via a callback
 * once the user switches views, hides the tab, or leaves the page. Mirrors
 * the same hook built for the to-do and e-learning apps — same
 * accumulate-while-visible pattern, same minimum-seconds threshold — and
 * is callback-based since this app doesn't have a global logger to write
 * through directly (CalculatorContext owns the actual Odoo sync call).
 */
