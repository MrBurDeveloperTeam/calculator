...
const inputClass = 'w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-800 placeholder:text-slate-400 transition focus:border-tiffany-600 focus:outline-none focus:ring-2 focus:ring-tiffany-600/20';
  ...
  return (
    <AuthShell centered>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div><div className="mb-1 flex items-center justify-between"><label htmlFor="login-password" className={labelClass}>Password</label><button type="button" onClick={() => setError('Password reset is not implemented yet.')} className="text-[10px] font-bold text-tiffany-600 hover:underline">Forgot Password?</button></div><input id="login-password" type="password" className={inputClass} placeholder="Password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete="current-password" /></div>
        <label className="flex items-center"><input type="checkbox" className="h-4 w-4 accent-tiffany-600" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} /><span className="ml-2 text-[11px] text-slate-500">Remember me</span></label>
      ...
      <p className="mt-6 text-center text-xs text-slate-500"><Link to="/register" className="font-bold text-tiffany-600 hover:underline">Don&apos;t have an account? Sign up</Link></p>
    </AuthShell>
  );
}
