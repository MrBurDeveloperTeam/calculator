  ...
  return (
    <div className={className}>
      ...
      <div className={`relative flex items-stretch rounded-xl shadow-sm ring-1 ring-slate-200 transition-all overflow-hidden ${!disabled ? 'focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-1' : 'opacity-60 bg-slate-50'}`}>
        ...
        {type === 'currency' && (
          <div className={`pointer-events-none flex items-center bg-slate-100 border-r border-slate-200 px-4 ${inputClassName}`}>
            <span className="text-slate-500 sm:text-sm font-bold whitespace-nowrap">{currencySymbol}</span>
          </div>
        )}
        ...
        {type === 'percent' && (
          <div className={`pointer-events-none flex items-center bg-slate-100 border-l border-slate-200 px-4 ${inputClassName}`}>
            <span className="text-slate-500 sm:text-sm font-bold whitespace-nowrap">%</span>
          </div>
        )}
      </div>
