...
const BottomControls: React.FC<BottomControlsProps> = ({ 
    ...
    return (
        <div className="absolute left-4 top-1/2 z-40 flex -translate-y-1/2 flex-col items-center">
            <button
              onClick={onOpenShop}
              className="pet-asset-no-tile relative group flex h-[70px] w-[70px] cursor-pointer flex-col items-center justify-center text-orange-500 transition-all duration-300 hover:scale-105 active:scale-95"
              title="Shop"
            >
               <AiOutlineShop
                    className="pet-asset-no-tile mb-0.5 h-8 w-8 transition-transform duration-300 group-hover:-rotate-6"
                    strokeWidth={10}
                />
               <span className="text-[11px] font-bold uppercase tracking-widest">Shop</span>
            </button>
