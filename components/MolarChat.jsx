...
export default function MolarChat({ isOpen, onClose, chatHistory, isChatLoading, chatInput, setChatInput, onSendMessage, onClearChat, onPetToggle, chatEndRef }) {
    ...
    const [config, setConfig] = React.useState({
        ...
        subtitle: 'Ready to assist with pricing, costs, margins, forecasts, and ROI planning.'
    });
            ...
            try {
                ...
                if (configs && configs.length > 0) {
                    setConfig({
                        ...
                        subtitle: configs[0].subtitle || 'Ready to assist with pricing, costs, margins, forecasts, and ROI planning.',
                    });
                    ...
                    if (promptData && promptData.length > 0) {
                        ...
                        setPrompts([
                            { text: "Review my cost structure", icon_name: 'Zap' },
                            { text: "Check procedure margins", icon_name: 'ShieldCheck' },
                            { text: "Find pricing risks", icon_name: 'AlertCircle' },
                            { text: "Forecast clinic profit", icon_name: 'BarChart3' },
                        ]);
                    }
                } else {
                    setPrompts([
            ...
            <div className="molar-chat-panel fixed bottom-4 right-4 md:bottom-6 md:right-6 w-[90vw] md:w-[400px] h-[70vh] md:h-[600px] max-h-[85vh] flex flex-col font-sans z-[9999] overflow-hidden rounded-[1.5rem] shadow-2xl shadow-slate-400/60 border border-white/40 bg-white/80 backdrop-blur-2xl ring-1 ring-slate-900/5">
                ...
                <div className="px-3 pt-1 relative z-20">
                    <a
                        ...
                        className="calculator-support-link group flex w-full items-center gap-3 rounded-2xl border px-3 py-2.5 text-left transition-all duration-200 focus-visible:outline-none active:scale-[0.99]"
                    >
                        <span className="calculator-support-icon flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border transition-transform duration-200 group-hover:scale-105">
                            <Mail className="h-5 w-5" aria-hidden="true" />
                        </span>
                        <span className="min-w-0 flex-1">
                            <span className="calculator-support-title block text-sm font-semibold">
                                Email Support
                            </span>
                            <span className="calculator-support-meta block truncate text-xs">
