...
// `calculatorPetRepository`'s confirmed-live atomic RPCs so the SAME game
...
// meowdoku_claim_check_in, meowdoku_get_achievements. The three older
// SQL-defined variants (meowdoku_get_progress, meowdoku_complete_level,
...
// uses z-[1000] — see `CalculatorVirtualPet.tsx`'s `extraGames` wiring in
...
// Todo. Closing this launcher does not unmount `SharedVirtualPet` — the
// pet room underneath is still open, matching Production's existing
...
export default function MeowdokuLauncher({ isOpen, onClose, userId }: MeowdokuLauncherProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    calculatorPetRepository.loadSnapshot(userId).then((snap) => {
  ...
  // Reward path replaced with `calculatorPetRepository`'s atomic RPCs —
  // the legacy launcher's local `setStats(prev => ({...}))` here is
  // exactly the pre-0.9.0 non-atomic pattern the shared migration must
  // not carry forward. Same numbers preserved: coin reward capped at
  // 1000, +15 happiness (via a load-then-save of the full snapshot,
  // since there is no standalone atomic happiness RPC —
  // `save_pet_snapshot`'s own coins argument is a documented no-op on an
  // existing row, so re-sending the just-mutated coins value here is safe
  ...
  const applyMeowdokuReward = async (rawCoins: unknown) => {
    ...
    if (reward <= 0) return;
    try {
      const nextCoins = await calculatorPetRepository.mutateCoins(userId, reward);
      setCoins(nextCoins);
      const snap = await calculatorPetRepository.loadSnapshot(userId);
      if (snap) {
        await calculatorPetRepository.saveSnapshot({
          ...snap,
  ...
  const applyMeowdokuSpend = async (amount: number, requestId: string) => {
    ...
    try {
      const nextCoins = await calculatorPetRepository.mutateCoins(userId, -amount);
      setCoins(nextCoins);
