...
export const PetRoom: React.FC<PetRoomProps> = ({ onNavigateToGame }) => {
  ...
  const getBedItem = (id: string | null) => {
    ...
    const shopBed = foodItems.find(
      (item) =>
        item.id === id &&
        item.category === 'Beds'
    );

    const fallback = BED_ITEMS.find(
      (bed) => bed.id === id
    );
        ...
        if (isOver && draggedTool) {
          ...
          if (draggedTool === 'soap') {
            if (!activeSoapType.current) {
              activeSoapType.current =
                'soap';

              soapConsumedOnRinse.current =
                false;

              const soapItem =
                getSoapItem();

              activeSoapMultiplier.current =
                Math.max(
                  1,
                  (soapItem?.hygiene || 50) /
                    50
            ...
            if (isSoapedUp) {
              setBubbles((prev) => {
                ...
                const rinsedBubbles =
                  remaining.length <=
                  RINSE_COMPLETE_THRESHOLD
                    ? []
                    : remaining;

                if (
                  rinsedBubbles.length <
                    prev.length &&
                  stats.hygiene < 100
                ) {
                  setStats((s) => ({
  ...
  const handleAppPointerUp = (
    ...
    if (isDraggingBall) {
      ...
      if (
        ...
        outsideWakeUntilRef.current =
          Date.now() +
          SLEEP_WAKE_DURATION_MS;
  ...
  const roomConfig =
    ROOM_THEMES[currentRoom];

  const activeBed =
    getBedItem(activeBedId);

  const bathroomProgressPercent =
    isSoapedUp
      ? bubbles.length <=
        RINSE_COMPLETE_THRESHOLD
        ? 100
  ...
  useEffect(() => {
    ...
    if (ballPlayTimer.current) {
      clearTimeout(
        ballPlayTimer.current
      );
    }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const current =
        outsidePetPosRef.current;

      const petWidth =
        192 * OUTSIDE_PET_SCALE;

      const petHeight =
        208 * OUTSIDE_PET_SCALE;
      ...
      const ballIsReleasedAndMoving =
        isBallMovingRef.current &&
        !isDraggingBallRef.current &&
        ballSpeed > 0.5;

      const hasReleasedBallTarget =
        !!outsideBallTargetRef.current ||
        ballIsReleasedAndMoving;

      const chaseTarget =
        hasReleasedBallTarget
          ? latestBallPos
          : null;
  ...
  return (
    ...
    >
              ...
              <div
                className={`transition-all duration-500 ${
                  isSleeping
                    ...
                    : 'filter drop-shadow-[0_0_25px_rgba(255,235,59,0.8)]'
                }`}
              >
                💡
              </div>
      ...
      {currentRoom ===
        ...
        (bubbles.length > 0 ||
          ...
          <div className="absolute top-[clamp(13rem,30vh,14rem)] left-1/2 -translate-x-1/2 z-50 flex flex-col items-center animate-in fade-in zoom-in-95 duration-300 pointer-events-none select-none">
            <div
              className={`w-48 h-2.5 bg-white/40 backdrop-blur-md rounded-full overflow-hidden shadow-lg ring-2 transition-all duration-500 ${
                isSoapedUp &&
                bubbles.length ===
                  MAX_BUBBLES
      ...
      >
        ...
        RoomType.PLAYROOM ? (
          ...
          >
            <Pet
              ...
              spriteSheetUrl={
                activePet.spriteSheetUrl
              }
              mouthPosition={
                activePet.mouthPosition
              }
              idleFrames={
                activePet.idleFrames
              }
              idleDuration={
                activePet.idleDuration
              }
              sleepInFrames={
                activePet.sleepInFrames
              }
              sleepHoldFrame={
                activePet.sleepHoldFrame
              }
              clickRow={
                activePet.clickRow
              }
              clickFrames={
                activePet.clickFrames
              }
              clickDuration={
                activePet.clickDuration
              }
              pose={outsidePetPose}
              onClick={
                handlePetClick
        ...
        {currentRoom ===
          RoomType.BATHROOM && (
            <div className="absolute bottom-10 right-10 opacity-50 text-6xl animate-float">
              🦆
            </div>
          )}
